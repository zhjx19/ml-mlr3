...
**本机实测收口数**（R 4.6.1 / Windows）：`verify_examples.R 20/20 PASS`、`check_answer_api.R` 幻觉键 0、harness 十节全过、`run_evals.mjs --selftest` 引擎 PASS。**已知限制**：ubuntu 那个 job 目前红在依赖步骤，病因是 runner 镜像里 `igraph.so` 缺系统库（不是本仓库的代码或清单问题，见 CHANGELOG 的收口记录）；它**不影响本 skill 的任何示例与正文口径**，windows job 与本机全绿。你自己的机器上遇到同类报错，按公告点名的包补系统依赖后重试即可——门禁不会替你装环境。
