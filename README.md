...
第一轮读数回来就把本轮自己的两个假设打掉了（windows job 首次全绿；ubuntu 仍红，公告是 `lines=1079 source=0 binary=0 win_unpacked=171`，三次子进程全 `rc=0`，而 `kknn` 在 1079 行里一次都没出现）：**"二进制没被用上、被一次 igraph 级编译拖爆"是错的**——`source=0`、171 个二进制解包，RSPM 的 Linux 通道完全在用，162s 花在下载上；那三条 C++11 note 也与本病无关。剩下唯一自洽的解释是"它压根没被请求安装"：清单按**装前**缺不缺算，而 runner 预装库里本来就有它，于是 `install.packages` 认定"已装/最新"、一字不提；等 171 个二进制落进高优先级库，它某个依赖被新版遮蔽，`requireNamespace(quietly = TRUE)` 又把真实报错整个咽掉——**"缺"这个字盖住了两种完全不同的病**（换通道 vs 处理遮蔽，修法不一样）。所以补的是那一刀：子进程同一进程内报 `BEFORE_INSTALL` / `AFTER_INSTALL` 逐包状态，父会话把"装前能加载、装完反而加载不了"单独点名，并对每个翻转型包当场 `loadNamespace()` 把**原文**贴出来。真因拿到之前，不改安装语义、不放宽任何断言。
