...
**第四次量尺量到了安装步骤自己头上。** 前三次都是"清单算错了"，而 ubuntu job 至今仍红在装依赖那一步——公告只有 `硬依赖安装失败: kknn`，编译错误的原文一行都没出来：`install.packages` 的输出只落在 Actions 日志里，而 `/logs` 要仓库权限（403），匿名读者只有 annotations。现在安装走子进程、输出被收进文件，于是这条步骤自己会说话：日志里三种安装形态分开计数（Linux 源码 / RSPM 二进制 / Windows 已解包，用来判断二进制通道到底有没有被用上）、子进程自报**装进了哪个库、用的哪套仓库**（区分"装到别处去了"和"真的装不上"）、缺失的硬依赖**自动重试一轮**（一轮就装上 = 瞬时故障，两轮都缺 = 确定性失败），失败时把报错行连上下文（带行号）贴成 `::error::`，并按传递依赖点名真凶——`kknn` 往往只是受害者，编译失败的是它的依赖。顺手把"取不到仓库索引"从静默返回空改成明说的 `::warning::`：一个谎称"什么都不缺"的绿灯，比红难查十倍。**病因仍未定位**，这一步只是让下一次 push 的 annotation 里第一次有原文可读；清单本身不会因此放宽——为了让灯变绿而改断言，等于把尺子锯短。

第一轮读数（`4d01eab`）先打掉本轮自己写下的假设：**"二进制没被用上、被一次 igraph 级编译拖爆"是错的**——`source=0` 而 `win_unpacked=171`，RSPM 的 Linux 二进制完全在用，162s 全花在下载上。当时剩下的解释是"遮蔽"，而第二轮读数（`a6bd7e9`）把它也打掉了：`BEFORE_INSTALL: … kknn=MISS` 说明它**装前就加载不了**（翻转类是空的），而 `why_missing()` 贴出的原文指向 `/home/runner/work/_temp/Library/igraph/libs/igraph.so`——**磁盘上有、却 dlopen 不了**。这一下把所有对上的线索都串起来了：`install.packages` 只看"装没装过"，装了就直接跳过（所以 1062 行日志里 `kknn` 零出现、一行报错都没有、三次 `rc=0`，重试对这类病结构性无效），而 `closure_missing()` 报"什么都不缺"**不是谎、是口径不对**——它量磁盘，这个病在加载。最贵的是那句原文自己断了：`annotate()` 只在元素之间拼 `%0A`，`conditionMessage` 自带的裸换行把 `::error::` 从中间劈断，真凶（第二行的系统库名）就这么丢了。这一轮补五样：`flat()` 先摊平任何进 annotation 的字符串；头条按**磁盘上有/没有**分两类报（"没装上 → 查日志"和"已装但加载不了 → 查系统库"是两种修法）；`unloadable_deps()` 往下钻一层点名真正加载不了的依赖；`ldd_missing()`（unix）对它 `ldd` 一遍，把缺的系统库名一并贴出——省掉"再红一轮才知道该装什么"；子进程装前装后各做一次**全库普查** `UNLOADABLE_BEFORE/AFTER`，清单全绿也照样报（库里躺着一个加载不了的依赖，就是下一次换包就红的潜伏病灶）。教训一句话：**"剩下唯一自洽的解释"这种话本身就是危险信号**——那是推断被我写成了结论，而否掉它的两行读数当时已经在手里。安装语义与断言一个字未放宽。本机实测：`scripts/test_install_logged.R` 九节全过（它又抓到两条真 bug：`why_missing()` 成功分支不挂包名，结果整句话被当成包名喂给 `ldd_missing()`；普查不过滤会把 `1.2`、`4.0` 这种版本号当成包名）、`--install` 干跑 `need=20 todo=0` rc=0、`--mirror` rc=0、`verify_examples.R 20/20 PASS`（18s）、`check_answer_api.R 168 个唯一键 0 幻觉`。**下一次 push 的 annotation 应该直接给出缺的那个系统库名**——拿到它才谈修环境，而不是继续猜。

第三轮（`0e63c74`）的红却是另一种病：**两个平台一起红，而公告只有一句 `object 'bundled' not found`**。上一轮加的全库普查引用了常量 `bundled`，它却定义在扫描段（第 244 行），而锚点安装发生在扫描**之前**（第 225 行）——R 脚本顶层的顺序就是语义。本机四道门禁当时全绿，因为 `todo=0`、锚点早已在场，`install_logged()` 这条路径**压根没被调用**；harness 前九节的验证方式又是"把定义抽进新环境再跑"，天生看不见顺序。两条盲区各补一样：`bundled` 上移到调用点之前，harness **§10 按顶层表达式顺序**断言 `install_logged()` 用到的 5 个常量全部早于首个调用点（顺带断言"一个调用点都没有 = 结构变了，这条尺得重写"）；新增 `--install-probe <包>`，让本机**复演只有 CI 干净机器才走的那条路**（临时库放 `.libPaths()[1]`，只装一个包，绝不动用户库，不进 CI）。`gates.yml` 的装依赖步骤也拿到和回归、体检两步同一条纪律：不走管道、落盘回显，**没跑到 `::notice::deps` 回执 = R 会话中途死掉**，把日志尾部逐行写成 `::error::`——否则脚本自带的病因注解根本没来得及打印，红灯又只剩一个状态灯。复演第一次跑就带回一条真发现：`--install-probe fastmap` rc=0，回执指向临时库，而全库普查报 `UNLOADABLE_BEFORE/AFTER: RWekajars rJava rjags`——本机磁盘上躺着三个加载不了的包（rJava 缺 JVM、rjags 缺 JAGS），和 ubuntu 的 `igraph` 是同一类病。它们不进清单，因为没有示例用到它们；但"换包就红"的潜伏病灶已经被点名，这条普查不是给 CI 写的装饰。断言与安装语义仍一个字未放宽；ubuntu 要的那个系统库名**这轮仍未取得**（它死在诊断之前）。
