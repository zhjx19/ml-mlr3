...
## [Unreleased] — 待发布为 v2.4.0 · 门禁给自己量了四次尺，第四条线收在边界内

### Changed（本轮收尾：README 去过程叙事 + 补回本项目自己的版本号）
- **README 里那三大段门禁 internals 删掉了**（依赖清单怎么从手抄改成算出来、硬软锚点账、`--mirror` 预飞、通道纯度、四轮安装诊断的逐轮读数）。为什么改：用户 2026-09-26 直接指出"这些没什么用的东西为什么还要放在 README 里"——那是**我做门禁的过程**，不是**用户装它要知道的契约**；细节在 CHANGELOG 里一条没少，README 只留四行命令、一句"红灯自带病因"、一句"环境只报不修"、一行已知限制。**顺手纠一个我自己外推错了的规矩**：用户此前说的是"别放 mlr3 / mlr3pipelines 包更新说明（NEWS）里的版本号"，我执行成了"本项目不提版本号"，结果仓库有 tag（`v2.1.0`/`v2.2.0`/`v2.3.0`）有 CHANGELOG，README 首屏却一个版本号都没有——公开读者无从判断自己装的是哪一版。现在补回：README 新增「版本与更新历史」段（当前 `v2.3.0`，下一版候选 `v2.4.0`）+ 首屏取最新 tag 的 `version` 徽章 + `SKILL.md` frontmatter `version: 2.3.0`。**并且给这件事钉了一道尺**：`scripts/verify_examples.R` 开头新增一条不计入用例数的元数据自检，比对 SKILL.md frontmatter / README 版本段 / CHANGELOG 最新已发布条目三处，不一致直接红——理由是这个仓库历史上真出过"两份平行的版本元数据各说各话"（那份 marketplace 声明已归档移除），版本号抄在两处就会分家是结构性风险而不是巧合。上游包 NEWS 版本号不进正文这条规矩不变：正文只留一行实测环境记录 + 当场探测的纪律。本机实测：`verify_examples.R` 自检 `[OK] 版本自洽 v2.3.0` + `20/20 PASS`、`check_answer_api.R` 幻觉键 0、`test_install_logged.R` 十节全过、`run_evals.mjs --selftest` 引擎 PASS。
