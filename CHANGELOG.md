...
- **README「已知限制」一句失实修正**。为什么改：原文说 ubuntu 缺 `libglpk.so.40` "不影响任何示例与正文口径"，但 CI 公开公告显示该缺库令 `igraph.so` 无法 `dlopen` → `classif.kknn` 加载失败（`AFTER_INSTALL: kknn=MISS`），而用例 `ppl(branch) 分支调参` 正依赖 `classif.kknn`，在 ubuntu 上无法执行。改为如实写明"影响面 = 该用例在 ubuntu 无法执行"，并说明已在 workflow 显式补系统前置，Ubuntu 已由 CI run #11 验证转绿。
...
### Added
- **`README.en.md` + 双语互链**：中英首行 `<sub>🌐</sub>` 互链。为什么加：首屏 `description` 已改英文以利跨 runtime 检索，英文读者点进来却只有中文 README；现补齐同结构的英文版，内容与中文版逐节对应（版本段同样声明 v2.4.0）。
...
- **`gates.yml` 系统前置步骤**：ubuntu job 在装依赖前 `apt-get install -y libglpk40`，把系统级前置写成明面上的备料步骤，而非静默修断言。**已在 CI run #11 验证 ubuntu 转绿。**
...
- 线上 CI（run #11 @ `e4e3566`）：ubuntu / windows / selftest **三 job 全 success**，系统前置 `libglpk40` 在 CI 上验证通过（此前 ubuntu 的灯为红）。
...
### 前序（并入 v2.4.0）：门禁给自己量了四次尺，第四条线收在边界内

### Changed（本轮收尾：README 去过程叙事 + 补回本项目自己的版本号）
- **README 里那三大段门禁 internals 删掉了**（依赖清单怎么从手抄改成算出来、硬软锚点账、`--mirror` 预飞、通道纯度、四轮安装诊断的逐轮读数）。为什么改：用户 2026-09-26 直接指出"这些没什么用的东西为什么还要放在 README 里"——那是**我做门禁的过程**，不是**用户装它要知道的契约**；细节在 CHANGELOG 里一条没少，README 只留四行命令、一句"红灯自带病因"、一句"环境只报不修"、一行已知限制。**顺手纠一个我自己外推错了的规矩**：用户此前说的是"别放 mlr3 / mlr3pipelines 包更新说明（NEWS）里的版本号"，我执行成了"本项目不提版本号"，结果仓库有 tag（`v2.1.0`/`v2.2.0`/`v2.3.0`）有 CHANGELOG，README 首屏却一个版本号都没有——公开读者无从判断自己装的是哪一版。现在补回：README 新增「版本与更新历史」段 + 首屏取最新 tag 的 `version` 徽章 + `SKILL.md` frontmatter 的 `version:`，三处随本版定为 `2.4.0`。**并且给这件事钉了一道尺**：`scripts/verify_examples.R` 开头新增一条不计入用例数的元数据自检，比对 SKILL.md frontmatter / README 版本段 / CHANGELOG 最新已发布条目三处，不一致直接红——理由是这个仓库历史上真出过"两份平行的版本元数据各说各话"（那份 marketplace 声明已归档移除），版本号抄在两处就会分家是结构性风险而不是巧合。上游包 NEWS 版本号不进正文这条规矩不变：正文只留一行实测环境记录 + 当场探测的纪律。本版 tag `v2.4.0` 已于 2026-09-26 发布（CI run #11 三 job 全绿后打）。**本版推前实跑**：`verify_examples.R` 元数据自检 `[OK] 版本自洽 v2.4.0` + `20/20 PASS`（连跑两次：合计 78s / 52s，同一套上次记的是 25s——机器变慢的原因没查，用例数与断言一条没减，计时账本来就为此而留）、`check_answer_api.R 168 个唯一键 0 幻觉`、`test_install_logged.R` 十节全过（含 §10 定义顺序尺）、`--install` 干跑 `need=20 todo=0` 且 `cran_release core=10 dev=-`、`run_evals.mjs --selftest` 引擎 PASS。

### Added（本轮：安装步骤自诊断，已收口）

- **安装步骤自带病因；三轮线上读数后收口（用户 2026-09-26 定的边界：环境这块门禁不自作主张）**。起点：ubuntu job 连续红在装依赖，而公开通道只有一句 `硬依赖安装失败: kknn`（`/logs` 要仓库权限 403，匿名读者只有 annotations），也就是本 skill 那条"红灯必须自带病因"的纪律在门禁自己的安装步骤上是失效的。于是把安装改成子进程 + `system2(stdout=TRUE, stderr=TRUE)` 收日志，让它自己说话：`TARGET_LIB` / `REPOS_USED(option)` / `REPOS_REQUESTED` 三条回执（区分"装到别处去了"和"真的装不上"，且**配置里的仓库**与**实际请求的仓库**各自 collapse 成一行——早先一句 `paste()` 会按元素循环，把两串长度不同的仓库配成假一对，公告里读到的 `posit.co 配 r-universe` 就是这么来的假数据）、装前/装后逐包可加载状态 `BEFORE_INSTALL` / `AFTER_INSTALL`、全库普查 `UNLOADABLE_BEFORE/AFTER`（清单全绿也照样 `::warning::`）、失败时按**磁盘上有/没有**分两类点名 + `unloadable_deps()` 往下钻一层 + `ldd_missing()`（unix）把缺的系统库名一起贴出。**三轮读数各自推翻一个我此前写下的结论**，这是这条线唯一的收获：① "RSPM 二进制没被用上、被一次 igraph 级编译拖爆"——错，`source=0` / `win_unpacked=171`，162s 全花在下载；② "kknn 装前可加载、装完被高优先级库遮蔽"——也错，`BEFORE_INSTALL` 那一刻它已经是 MISS；③ 真因是 **`igraph.so` dlopen 失败 = 缺系统库**，而 `install.packages` 只看"装没装过"、装了就直接跳过，所以 1062 行日志里 `kknn` 零出现、一行报错都没有、三次 `rc=0`、重试对这类病结构性无效。**"缺"这个字底下压着三种不同的病**（没装上 → 查日志；装前能加载装完加载不了 → 库遮蔽；磁盘上有却 dlopen 不了 → 运行环境缺系统库），`closure_missing()` 那句"传递依赖里还缺: 无"不是谎、是**口径不对**：它量磁盘，这个病在加载。第三条病在 README 里换成了一句给用户的话（缺什么就点名什么，你自己装完再重试）。**第四轮的红是我自己造的**：`0e63c74` 让 ubuntu 和 windows 一起红，公告只有 `object 'bundled' not found`——普查代码用了定义在它之后（扫描段第 244 行）的常量，而锚点安装在第 225 行；本机 `todo=0` 压根不调用 `install_logged()`，四道门禁于是全绿。修法是把 `bundled` 上移，并补两样让这类"CI-only 路径本机不跑"的盲区以后藏不住：harness **§10** 按**顶层表达式顺序**断言 `install_logged()` 用到的 5 个常量全部早于首个调用点（前面九节"把定义抽进新环境 eval"的验证方式天生看不见顺序问题），以及 `--install-probe <包>`——本机**复演**那条干净机器才走的路（临时库放 `.libPaths()[1]`，只装一个包，绝不动用户库，不进 CI）。复演第一次跑就量出一条真发现：`UNLOADABLE_BEFORE/AFTER: RWekajars rJava rjags`，本机磁盘上一直躺着三个加载不了的包（`rJava` 缺 JVM、`rjags` 缺 JAGS），和 ubuntu 的 `igraph` 同一类病。**过程中 harness 抓到三条真 bug，全部是跑出来的不是读出来的**：`why_missing()` 初版写成 `loadNamespace(p, character.only = TRUE)`（那是 `library()` 的参数）→ 对任何包稳定返回 "unused argument"，一条看着像真病因的假病因；`deparse()` 对长/命名向量返回**多行**字符向量，塞进 `sprintf` 会按元素循环，把一条 `install.packages(repos = c(...))` 劈成两条碎语句（症状是 rc≠0 + `unexpected symbol`，极易误诊成"仓库坏了"）；`why_missing()` 成功分支不挂包名，于是整句话被 `sub()` 当成包名喂给 `ldd_missing()`。还有三条是环境类自伤：`system2(env=…)` 在 Windows 会把命令行拼歪（rc=5）；`--vanilla` 连 `.Rprofile`/`.Renviron` 一起跳过，而 runner 的二进制通道正靠它生效；`R.home("bin")` 在 Windows 是 `bin/x64`。产物原先直接写在仓库工作目录，本机跑一次多出一堆未跟踪的 `install_*.log`（已改 `tempfile()`，清走 6 份遗留）。**收口决定与最后一次读数**：用户 2026-09-26 定了边界——环境搭建（含系统库）门禁不自作主张，只点名缺什么，由用户自行装好再重试。`91a482d`（含上面那两样修复）推上去后读数一次到位：**windows job 恢复全绿**（`todo=4`、`20/20 PASS` 合计 30s、`UNLOADABLE_BEFORE/AFTER` 两端皆空、`soft_anchor mlr3extralearners = 1.7.0` 走的是 CRAN 版）；ubuntu 的公告把真凶整句给了出来：`[kknn] 已安装但加载不了 … igraph 加载失败原文：unable to load shared object '/home/runner/work/_temp/Library/igraph/libs/igraph.so': libglpk.so.40: cannot open shared object file` + `ldd[igraph] 缺的系统库: igraph.so: libglpk.so.40 => not found`，而且普查给出了因果顺序：装前 `UNLOADABLE_BEFORE:` 为空、装后变成 `igraph` —— 是**这次从 RSPM 解包进来的那个 igraph 二进制需要 `libglpk.so.40`，镜像里没有**。三条尺（`why_missing` + `unloadable_deps` + `ldd_missing` + 全库普查）在同一条公告里互相印证，这条线到此为止：**它不改 skill 正文一个字**，README 只留一行可执行的已知限制（`libglpk.so.40` 在 Debian/Ubuntu 上由 `libglpk0` / `libglpk-dev` 提供，这一对应按发行版惯例给出、未在 runner 上实装验证）。**本机实测收口数**：`test_install_logged.R` 十节全过、`--install-probe fastmap` rc=0（91 行日志）、`--install` 干跑 rc=0（`need=20 todo=0`，`cran_release core=10 dev=-`）、`--mirror` rc=0、`verify_examples.R 20/20 PASS`（合计 25s / 墙钟 25s）、`check_answer_api.R` 幻觉 0、`run_evals.mjs --selftest` 引擎 PASS。
