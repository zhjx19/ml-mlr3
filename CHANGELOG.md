...
## [Unreleased] — 2026-09-26 · 门禁给自己量了四次尺

### Added（本轮：安装步骤）
- **安装步骤自带病因（`scripts/list_example_deps.R` 的 `install_logged()`）**。为什么改：ubuntu job 红在装依赖那一步，公告只有一句 `硬依赖安装失败: kknn`，而编译错误原文一行都没出来——`install.packages` 的输出只落在 Actions 日志里，`/logs` 要仓库权限（403），匿名读者只有 annotations。也就是说本 skill 那条"红灯必须自带病因"的纪律，在门禁自己的安装步骤上是失效的。现在安装走子进程 + `system2(stdout = TRUE, stderr = TRUE)` 收日志，这一步自己会说话：① 三种安装形态分开计数（Linux `* installing *source* package` / RSPM `* installing *binary* package` / Windows `successfully unpacked`）——ubuntu 上若 source 段占绝大多数，就说明 P3M/RSPM 的二进制没被用上，一次 `igraph` 级编译足够把步骤拖爆；② 子进程自报 `TARGET_LIB` 与"仓库（option）/ 实际请求"两条回执，用来区分**装到别处去了**和**真的装不上**（本机默认 `getOption("repos")` 是占位符 `@CRAN@`，只报它会在 CI 上读成"仓库是个不存在的东西"）；③ 缺失的硬依赖**自动重试一轮**，一轮就装上 = 瞬时故障、两轮都缺 = 确定性失败，一次运行就把这两种病因分开；④ 失败时先点名**传递依赖**里的缺口再贴每包日志段，最后把报错行连上下文（带行号）贴成 `::error::`——`kknn` 常常只是受害者，编译失败的是它的依赖（实测 `kknn` 的闭包在干净机器上缺 12 个，含 `igraph`）。另把 `closure_missing()` 在拿不到仓库索引时的静默返回空改成明说的 `::warning::`：**一个谎称"什么都不缺"的绿灯比红难查十倍**（真实触发过一次：`available.packages(filters = c("Depends","Imports","LinkingTo"))` 报 `invalid 'filters' argument`，被外面的 `tryCatch(NULL)` 吞掉，于是"传递依赖里还缺: 无"）。**顺手修掉的自伤（全在真跑里抓到，不是读代码读出来的）**：`deparse()` 对长/命名向量返回**多行**字符向量，塞进 `sprintf` 会按元素循环，把一条 `install.packages(repos = c(...))` 劈成两条碎语句——症状是 rc≠0 加一句 `unexpected symbol`，极易误诊成"仓库坏了"；`system2(env = c(Sys.getenv(), R_LIBS = ...))` 在 Windows 上会把整条命令行拼歪（rc=5）；`--vanilla` 会连 `.Rprofile`/`.Renviron` 一起跳过，而 runner 的 P3M/RSPM 二进制通道正靠它生效——用了它等于在 ubuntu 上强制退回纯源码编译；`R.home("bin")` 在 Windows 是 `bin/x64`，那里只有 `Rscript.exe`；`attr(,"status")` 只在非零退出时挂；产物原先直接写在仓库工作目录，本机跑一次多出一堆未跟踪的 `install_*.log`（已改 `tempfile()`，并清走 6 份历史遗留）。验证方式：`scripts/test_install_logged.R`（**已入库**，验证资产随仓库继承，不留在这次会话的临时目录里）把脚本里的定义抽出来跑，覆盖 临时库落地 / 退出码 / 长 repos 回归 / 两种日志风味的 `pkg_block` / `err_window` / 空库桩下的闭包 / 索引不可用降级，全部断言通过。它**不进 CI**：要联网装包、还依赖镜像当天状态，把它挂进门禁等于给CI 添一个新的假红源——这条判据和本轮 `--mirror` 的处置一致。**ubuntu 的真实病因仍未定位**：公开证据排除了上游（CRAN 的 r-devel/r-release debian-clang/gcc、fedora、windows、macos 上 `kknn 1.4.1` 全 OK）也排除了"放宽断言"这一选项；那三条只在 ubuntu 出现的 C++11 note 与 165s 的窗口，在拿到本次 instrumentation 的 annotation 之前无法归因。**这一步不修根因，它只让下一次 push 第一次有原文可读。**
- **线上第一轮读数（同一夜，`4d01eab` 的 run `36220557575`）——旧假设当场阵亡，新抓手补上**。读到的公告：windows job 首次全绿（装依赖 36s、回归 37s、幻觉体检 8s）；ubuntu 仍红在装依赖（162s），公告是 `install log lines=1079 source=0 binary=0 win_unpacked=171 rc=anchor:0 deps:0 retry:0` + `[kknn] 日志里没有它的安装段` + `日志里没有匹配到任何报错行` + `硬依赖安装失败: kknn`，回执 `TARGET_LIB: /home/runner/work/_temp/Library`。这一组数字直接**推翻本轮自己写下的两个假设**：① "二进制没被用上、一次 igraph 级编译把步骤拖爆"——`source=0` 而 `win_unpacked=171`，RSPM 的 Linux 二进制完全在用（171 个包按二进制解包，零编译），162s 花在下载上；② 那三条 C++11 note 与本病无关，因为这一轮压根没编译。剩下的唯一自洽解释是：**kknn 从一开始就没进请求清单**——`todo` 按"装前缺不缺"算，所以它在装前是**可加载**的（runner 预装库里有它，`install.packages` 因此认为"已装/最新"而一字不提），装完 171 个二进制进 `[1]` 库之后反而加载不了（它的某个依赖被高优先级库里新装的那版遮了）。而 `requireNamespace(quietly = TRUE)` 把真实原因整个咽掉——这就是"缺"这个字盖住的两种不同的病，修法完全不同（换通道 vs 处理遮蔽）。所以这一轮加的是那一刀：子进程在同一进程内报 `BEFORE_INSTALL:` / `AFTER_INSTALL:` 逐包可加载状态；父会话比对装前/装后，把"装前能加载、装完反而加载不了"单独 `::error::` 点名，并对每个翻转型包当场 `loadNamespace()` 一次把**原文**贴出来（新增 `why_missing()`）。顺带修掉回执自己的一个假数据：`paste("REPOS_USED:", getOption("repos"), "| 实际请求:", repos_all)` 会按**元素**循环，长度不同时把"配置里的第 i 套仓库"和"请求的第 i 套仓库"配成假一对——公告里读到的 `posit.co 配 r-universe` 就是这么来的，现在两边各自 collapse 成一行、拆成 `REPOS_USED(option)` 与 `REPOS_REQUESTED` 两条。**harness 在这一轮抓到一条真 bug**：`why_missing()` 最初写成 `loadNamespace(p, character.only = TRUE)`（那是 `library()` 的参数），于是它对任何包都稳定返回 "unused argument" ——一条看着像真病因的假病因，正好是这套工具存在的理由。**这一轮不改安装语义**：没弄清真因之前不动 `lib=`、不加重装、不放宽任何断言。本机实测四道门禁全绿：`verify_examples.R 20/20 PASS`（合计 17s）、`--install` 干跑 rc=0（`need=20 todo=0 retry=0`）、`--mirror` 逐条有来源、`run_evals.mjs --selftest` 引擎 PASS。
- **线上第二轮读数（`a6bd7e9`）——"遮蔽"假设也阵亡；真因换了一层：dlopen**。读到的：`install log lines=1062 source=0 binary=0 win_unpacked=171 rc=anchor:0 deps:0 retry:0`；回执 `BEFORE_INSTALL: bestNormalize=MISS e1071=MISS kknn=MISS xgboost=MISS` → `AFTER_INSTALL: bestNormalize=OK e1071=OK kknn=MISS xgboost=OK`，重试子进程 `BEFORE_INSTALL: kknn=MISS` → `AFTER_INSTALL: kknn=MISS`（仍 `rc=0`）；`[kknn] 日志里没有它的安装段 | kknn 加载失败原文：unable to load shared object '/home/runner/work/_temp/Library/igraph/libs/igraph.so':` —— **然后句子就断了**。三条读数一次定死三件事：① 上一轮写的"遮蔽"没有证据支持：`pre` 那一刻 kknn 已经是 MISS，翻转类是空的，而我说"唯一自洽的解释"时手里的 BEFORE/AFTER 恰恰否掉了它；② kknn **磁盘上有**（报错指向它的文件路径而不是 "no package called"），所以 `install.packages` 认定"已装"直接跳过——这就是"日志 1062 行零报错、三次 rc=0、它却永远装不上"的全部成因，重试对这类病结构性无效；③ 病在 `igraph.so` 加载不了 = **缺系统库**，不是缺 R 包。于是 `closure_missing()` 那句"传递依赖里还缺: 无"**不是谎，是口径不对**：它量磁盘，这个病在加载。句子断掉是本轮最贵的一条：`annotate()` 只在**元素之间**拼 `%0A`，`conditionMessage` 自带的裸换行原样漏进 `::error::`，工作流命令按行解析，于是真凶（第二行的系统库名）被从中间劈掉。**这一轮补的五样**：① `flat()`——任何进 annotation 的字符串先摊平，harness §3 用一条两行的 dlopen 假原文钉住回归；② 头条从"硬依赖安装失败"改成按**磁盘上有/没有**分两类报（"没装上 → 查日志"与"已安装但加载不了 → 查运行环境/系统库"是两种修法），"日志里没有它的安装段"那一句也补上机制说明（磁盘上已存在 → 被 `install.packages` 跳过 → 重试无效）；③ `unloadable_deps()` 往下钻一层，把坏包的直接依赖逐个探并点名真正加载不了的那个（本次会是 `igraph`）；④ `ldd_missing()`（仅 unix）对它 `ldd` 一遍，把 "not found" 的系统库名一并贴出——省掉"再红一轮才知道该装什么"；⑤ 子进程装前装后各做一次**全库普查** `UNLOADABLE_BEFORE/AFTER`（只查各包自己声明的 `Imports/Depends`，差集说明是哪一步把谁弄坏的），并且**清单全绿也照样 `::warning::`**：库里躺着一个加载不了的依赖 = 下一次换个包就红的潜伏病灶。**harness 又抓到两条真 bug**（都是它跑出来的，不是读代码读出来的）：`why_missing()` 的成功分支不挂包名，而头条用 `sub(" 加载失败原文.*", "", ud)` 取名字，于是整句话被当成包名传给了 `ldd_missing()`——现在两个分支都以包名开头，§7 断言 `startsWith(ud, paste0(nm, " "))`；普查解析没过滤时会把版本号 `1.2 / 4.0 / 0.1` 当成包名漏出来（`Matrix (>= 1.2)` 剥掉括号后剩下它们），现在先按包名字形 `^[[:alpha:]][[:alnum:].]*$` 过滤（§9 测这条）。普查刻意用 `chartr` + `"[, ]+"` 而不是带反斜杠的正则：拼子进程脚本时反斜杠最容易少一层。§7/§8 用桩库（手写 `installed.packages` 矩阵 + `requireNamespace` 桩）造"装着但坏了"的现场；`ldd_missing` 的 unix 分支本机跑不到（Windows），只断言它不炸、返回字符向量，**真效果标注为下一次线上见**。清单与断言一个字未放宽。本机实测：`test_install_logged.R` 九节全过、`--install` 干跑 rc=0（`need=20 todo=0`）、`--mirror` rc=0、`verify_examples.R 20/20 PASS`（合计 18s）、`check_answer_api.R 168 个唯一键 0 幻觉`、`run_evals.mjs --selftest` 引擎 PASS。
- **线上第三轮读数（`0e63c74`）——红换成我自己造的：定义顺序，而且把唯一全绿的 windows job 一起带红了**。公告只有一句 `object 'bundled' not found`（windows check-run `108376098439` = failure；ubuntu 同因），也就是说**上一轮辛苦装的那套诊断本轮一个字都没输出**——它死在自己前面。病因：第二轮给 `install_logged()` 加的全库普查引用了常量 `bundled`，而这个常量定义在扫描段（原第 244 行），锚点安装却发生在扫描**之前**（第 225 行）——R 脚本顶层的执行顺序就是它的语义。**为什么本机四道门禁全绿抓不到**：本机 `todo=0`、锚点早已在场，`install_logged()` 这条路径压根没被调用（CI 干净机器第一步就走它）；而 harness 前九节的验证方式是"把定义抽进一个新环境再跑"，这种做法天生看不见顺序问题——两类盲区各补一样：① `bundled` 上移到 `installed` / `install_logged` 之前，并把这段顺序教训写进注释；② harness **§10** 单独按**顶层表达式顺序**量，断言 `install_logged()` 用到的 5 个常量（`bundled` / `repos_all` / `rscript_bin` / `artifact_dir` / `dep1`）的定义位置全部早于首个调用点，同时断言"一个调用点都没有 = 结构变了，这条尺得跟着重写"（否则它会安静地永远通过）。③ 新增 `--install-probe <包>`：让本机**复演只有 CI 干净机器才走的那条路**——临时库放 `.libPaths()[1]`、只装一个包、**绝不动用户库**，把 `TARGET_LIB / REPOS_* / BEFORE_INSTALL / AFTER_INSTALL / UNLOADABLE_*` 五类回执打出来就退出；它不进 CI（判据同 `--mirror`）。④ `.github/workflows/gates.yml` 的装依赖步骤拿到和 step 5/6 同一条纪律：不走管道、重定向落盘再 `cat`，**没跑到 `::notice::deps` 回执 = R 会话中途死掉**，把日志尾部逐行写成 `::error::`。三种情形用假日志在 `set +o pipefail` 下实测：有回执且 rc=0 不出兜底、rc=1 但脚本自己报了病因不出兜底、rc=1 且无回执出兜底并贴出 `object 'bundled' not found` 那一行。**复演第一次跑就带回一条真发现**：`--install-probe fastmap` rc=0、日志 91 行，`TARGET_LIB` 指向临时库、`BEFORE/AFTER_INSTALL: fastmap=OK`，而全库普查报 `UNLOADABLE_BEFORE/AFTER: RWekajars rJava rjags`——本机磁盘上一直躺着三个加载不了的包（`rJava` 缺 JVM、`rjags` 缺 JAGS），和 ubuntu 的 `igraph` 是**同一类病**。它们不进清单，因为没有任何示例用到它们；但"库里躺着一个加载不了的依赖 = 下一次换个包就红"这条推理在本机当场被证实了，也就是说普查不是给 CI 写的装饰。**ubuntu 要的那个系统库名这轮仍未取得**（它死在诊断之前），断言、清单、安装语义一个字未放宽。本机实测：harness 十节全过、`--install` 干跑 rc=0（回执 `cran_release core=10 dev=-`）、`verify_examples.R 20/20 PASS`（合计 25s / 墙钟 25s）、`check_answer_api.R` 幻觉 0、`run_evals.mjs --selftest` 引擎 PASS。
...
### Changed
- **安装入口收敛为两条：skills.sh 与 GitHub**（README「快速开始」删去 Claude Code plugin marketplace 通道）。为什么改：三条通道里，marketplace 那条既没有实测过的安装回执，也要在仓库里额外维护一份和 SKILL.md 平行的版本元数据——历史上它已经出过一次"两个通道版本号各说各话"。入口越少，能兑现的承诺越实。**`.claude-plugin/` 已归档移除**（用户 2026-09-26 确认不走该通道）：它不被 README、四道门禁、`gates.yml` 任何一处引用，却仍携带一份和 SKILL.md 平行的版本元数据——留着就是下一次"两个通道版本号各说各话"的伏笔。仓库里不再有任何 marketplace 声明文件。

### Added
- **锚点账分硬软两级 + `--mirror` 预飞（门禁的第三次量尺）**。为什么改：上一轮那条修复**推上去之前**，把清单里的 20 个包逐个对着 TUNA 的 CRAN 索引问了一遍，结果推翻了我自己刚写下的前提——`mlr3extralearners` **不在 CRAN**：25158 个包的索引里没有它、包页返回 404，本机那份的 DESCRIPTION 里 `Repository` 字段直接是 NA、版本号 `1.7.0.9000`（GitHub 构建的形态），官方分发通道是 mlr-org 的 r-universe。照上一轮"锚点装不上直接红"的写法，CI 会在第一步就**第四次**钉红，而病因是门禁自己造的。现在分两级：硬锚点 `mlr3verse` / `mlr3learners`（CRAN 可得，装不上 = 这台机器不配跑门禁）；软锚点 `mlr3extralearners`（缺席只影响它注册的 `classif.lightgbm` / `regr.lightgbm` / `classif.catboost` 三个键，让它对应的断言记 SKIP，而不是把通道问题伪装成"文档有幻觉"）。`--install` 把 `mlorg = https://mlr-org.r-universe.dev` **追加到 `repos` 末位**，CRAN / RSPM 供得上的仍旧优先走原通道，只有它们没有的才落出去。verify 侧同步拆断言（用例 20：CRAN 基线组无条件 + 具名病因，extralearners 组 `requireNamespace()` 守卫、放在本例最后，免得 SKIP 把前面的基线断言一起吞掉）。两条探针支撑写法：该包**装了但没加载**时键就已经在字典里（`library(mlr3verse)` 下加载前后 `keys()` 都是 285 个）→ 守卫用 `requireNamespace()` 足够，不必 `library()`；universe 的 `bin/windows/contrib/4.6/PACKAGES`（38 个包）里有 `mlr3extralearners` → Windows job 不用当场编译。**教训一句话**：清单"算得出来"和"CI 那份仓库快照里装得上"是两件事——前者靠扫源码，后者必须去问仓库；本机装着 ≠ 本机装得上（本机那份当初就是从 GitHub 装的，所以这个洞在本机永远看不见）。新增 `--mirror [--repos 镜像]` 一条命令做这件事，不进 CI（免得镜像索引超时变成新的假红）。本机实测：`need` 20（hard 16 / opt 4）、`verify_examples.R 20/20 PASS`（合计 26s / 墙钟 27s）、`check_answer_api.R 168 个唯一键 0 幻觉`。SKILL.md 的「依赖包提示」同步补了软锚点那条通道的安装式，因为正文承诺过那几个键。顺手把仓库解析统一成一条 `repos_all`（`@CRAN@` 占位先落实成 `cloud.r-project.org` 再追加 universe——非交互的 CI 里带着占位符去装包会卡住），环境指纹那行 annotation 也改成印**实际生效的** repos（`CRAN=… mlorg=…`），否则"这次到底用了哪些通道"仍然只有跑的人自己知道。本机 `--install` 干跑（todo=0，不动库）与 `--mirror` 实跑均 rc=0。
- **通道纯度自检（预飞顺带查出来的第二个洞）**。为什么加：mlr-org 的 universe 那 38 个包（实测索引）里 `bbotk` / `mlr3` / `mlr3misc` / `mlr3tuning` 这些核心包**也在**，而且是 `.9xxx` 开发版——把 universe 追加进 `repos` 的那一刻，就存在"CI 悄悄装了另一套代码，annotation 看起来仍旧全绿"的风险。`repos` 顺序（CRAN/RSPM 在前）是唯一的屏障，但顺序是**意图**不是**证据**，所以 `--install` 装完复核 10 个核心包的版本号，命中 `^\d+\.\d+\.\d+\.9` 就 `::warning::`，并固定留一行可 grep 的 `::notice::cran_release core=10 dev=-`。判故不用 `\.9` 是因为 `mlr3misc 0.9.12` 这类正规 CRAN 版本号里也带 `.9`，那样判等于天天假告警。顺带把另一条事实记账：软锚点自己还拖一个 universe-only 的传递依赖 `mlr3cmprsk`（CRAN 上 404），所以那条通道不是"补一个包"而是"供两个包"。本机干跑：`repos = CRAN=https://cloud.r-project.org mlorg=https://mlr-org.r-universe.dev | soft_anchor mlr3extralearners = 1.7.0.9000 | cran_release core=10 dev=-`（软锚点本就允许是开发版，它不在核心名单里）。
